...
class Ion_auth_model extends CI_Model
{
    ...
    public function register($identity, $password, $email, $additional_data = [], $groups = [])
    {
        ...
        $query = $this->db->get_where($this->tables['groups'],
    ...
    public function login($identity, $password, $remember = false)
    {
        ...
        $query = $this->db->select($this->identity_column . ', email, first_name,last_name, id, password, active, last_login')
            ->where($this->identity_column, $identity)
            ->limit(1)
            ->order_by('id', 'desc')
            ->get($this->tables['users']);
    ...
    {
        $recheck = (null !== $this->config->item('recheck_timer', 'ion_auth')) ? $this->config->item('recheck_timer',
            'ion_auth') : 0;
    ...
    {
        if ($this->config->item('track_login_attempts', 'ion_auth') && $this->config->item('track_login_ip_address',
                'ion_auth')) {
    ...
    public function clear_login_attempts($identity, $old_attempts_expire_period = 86400, $ip_address = null)
    {
        if ($this->config->item('track_login_attempts', 'ion_auth')) {
            ...
            $old_attempts_expire_period = max($old_attempts_expire_period,
                $this->config->item('lockout_time', 'ion_auth'));
    ...
    public function update($id, array $data)
    {
        ...
        if (array_key_exists($this->identity_column,
                $data) && $this->identity_check($data[$this->identity_column]) && $user->{$this->identity_column} !== $data[$this->identity_column]) {
        ...
        if (array_key_exists($this->identity_column, $data) || array_key_exists('password',
                $data) || array_key_exists('email', $data)) {
    ...
    {
        if (isset($this->_ion_hooks->{$event}[$name]) && method_exists($this->_ion_hooks->{$event}[$name]->class,
